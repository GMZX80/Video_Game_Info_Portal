# Pages Artefact Review

Generated: 2026-06-18

The production GitHub Pages artefact is built from `dist/` only.

## Included Files

- `.nojekyll`
- `assets/css/site.css`
- `assets/data/claims.json`
- `assets/data/events.json`
- `assets/data/games.json`
- `assets/data/generated/credits-public.json`
- `assets/data/generated/evidence-index.json`
- `assets/data/generated/external-identifiers-public.json`
- `assets/data/generated/games-index.json`
- `assets/data/generated/games-public.json`
- `assets/data/generated/mobygames-index.json`
- `assets/data/generated/narrative-search-index.json`
- `assets/data/generated/north-east-collection.json`
- `assets/data/generated/organisations-index.json`
- `assets/data/generated/organisations-public.json`
- `assets/data/generated/people-index.json`
- `assets/data/generated/people-public.json`
- `assets/data/generated/places-public.json`
- `assets/data/generated/public-search-index.json`
- `assets/data/generated/releases-index.json`
- `assets/data/generated/releases-public.json`
- `assets/data/generated/search-index.json`
- `assets/data/generated/source-assertions-public.json`
- `assets/data/generated/source-items-index.json`
- `assets/data/generated/source-trail-public.json`
- `assets/data/generated/source-trails-public.json`
- `assets/data/generated/timeline-events.json`
- `assets/data/organisations.json`
- `assets/data/people.json`
- `assets/data/photos.json`
- `assets/data/places.json`
- `assets/data/relationships.json`
- `assets/data/sources.json`
- `assets/images/asset-montage.jpg`
- `assets/images/baer-patent-page.webp`
- `assets/images/favicon.svg`
- `assets/images/magnavox-odyssey.webp`
- `assets/images/newcastle-crt-hero.webp`
- `assets/images/ralph-baer.webp`
- `assets/images/tynesoft-team-group.webp`
- `assets/images/tynesoft-team-wide.webp`
- `assets/js/narrative.js`
- `assets/js/north-east-collection.js`
- `assets/js/site.js`
- `collections/index.html`
- `collections/north-east-collection/index.html`
- `contribute/index.html`
- `games/doctor-who-and-the-mines-of-terror/index.html`
- `games/index.html`
- `games/oxo/index.html`
- `games/super-gran/index.html`
- `index.html`
- `lineages/index.html`
- `magazines/index.html`
- `magazines/sinclair-user/index.html`
- `north-east-collection.html`
- `people/gary-partis/index.html`
- `people/index.html`
- `people/phil-scott/index.html`
- `phase-0/index.html`
- `places/blaydon/index.html`
- `places/index.html`
- `research/corrections/index.html`
- `research/index.html`
- `search/index.html`
- `sources/mobygames/index.html`
- `stories/code-through-the-letterbox/index.html`
- `stories/index.html`
- `studios/index.html`
- `studios/tynesoft/index.html`
- `talk/index.html`
- `timeline/index.html`

## Required Content Checks

- `index.html`: yes
- `north-east-collection.html`: yes
- required CSS and JavaScript: yes
- generated public JSON: yes
- required images: yes
- `.nojekyll`: yes
- root-relative `/assets/` paths: no

## Excluded Source Material

- `raw/`, `curated/`, `research/`, `reports/`, `scripts/`, `tests/`, caches, SQLite files and unresolved private notes are not copied.

## Validation Notes

- No Pages artefact boundary failures found.
