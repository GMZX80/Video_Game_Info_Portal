...
The canonical `credits.jsonl` file still contains 80 explicit source-linked credits. The public graph now adds local research credit rows, including two Phil Scott candidate rows, but the external integration deliberately does not auto-promote Wikipedia, Wikidata, MobyGames links, or secondary database rows into confirmed credits.

## Why Phil Scott Was Sparse

Before this branch, public search relied mostly on internal research records, curated source links, and magazine/source records. The site had MobyGames links for Phil Scott and Trolls, but not a local credit table.
...
- The reviewed manual CSV path now imports one Phil Scott `Trolls` row as a candidate secondary database credit.
- The Phil Scott page also displays the reviewed candidate Wikipedia row for `Winter Olympiad '88`.
